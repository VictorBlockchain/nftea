...
interface i1155 is IERC1155{
    ...
    function withdrawNFT(address _to, uint256 _vnft, address _vnftContract) external returns(bool);
...
contract VAULT {
  ...
  function withdrawNFT(address _to, uint256 _nft, address _nftContract) public returns(bool){
    require(_isC[msg.sender], 'you are not that cool');
    i1155(_nftContract).safeTransferFrom(address(this), _to, _nft,1,'');
    return true;
  }

  function approve1155(address _nftcontract, address _creator) public returns(bool){

    require(_isC[msg.sender], 'you are not that cool');
    i1155(_nftcontract).setApprovalForAll(_creator,true);
    return true;
  }
...
using SafeMath for uint256;
    ...
    function approveVaultTransfers(uint256 _nft, address _token, address _operator) public {

      require(i1155(NFTEA).balanceOf(msg.sender,_nft)>0,'you do not own this nft');
      address _contract = _N2_V[_nft];
      i1155(_contract).approveTransfers(_operator, _token);
      emit approveVaulT(msg.sender, _contract);
    }
    function approveVault1155(uint256 _nft, address _nftcontract, address _operator) public {

      require(i1155(NFTEA).balanceOf(msg.sender,_nft)>0,'you do not own this nft');
      address _contract = _N2_V[_nft];
      i1155(_contract).approve1155(_nftcontract, _operator);
      emit approveVaulT(msg.sender, _contract);
    ...
    function remveVaultNFT(uint256 _nft, uint256 _vnft, address _vnftContract) public returns(bool){
          require(i1155(NFTEA).balanceOf(msg.sender,_nft)>0,'you do not own this nft');
          require(_N2_S[_nft]._brew>block.timestamp, 'not time to brew');
          address _contract = _N2_V[_nft];
          i1155(_contract).withdrawNFT(msg.sender,_vnft,_vnftContract);
          return true;
    }
